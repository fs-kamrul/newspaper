...
//selecting records
$sql1="select  id, title, contain, image, modified from my order by id desc LIMIT 5";
...
$rshead=mysqli_query($conn,$sql1) or die($sql1.">>".mysqli_error($conn));
...
include('sql/config.php');
$id=$_REQUEST['id'];
$query=mysqli_query($conn,"select * from my LEFT JOIN reala ON my.title = reala.realtitle where reala.cid ='$id' order by  my.id desc") or die(mysql_error());
